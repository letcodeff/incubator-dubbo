...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
     ...
     * @param invocation  invocation 上下文
     * @param invokers    invoker candidates  经过路由过滤的服务提供者列表| invokers 列表可以看做是存活着的服务提供者列表
     * @param selected    exclude selected invokers or not 已经经过负载均衡被选出的提供者集合
    ...
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation,
        ...
        /*
           检测 selected 中是否包含 stickyInvoker。如果包含的话，
           说明 stickyInvoker 在此之前没有成功提供服务（但其仍然处于存活状态）。
           此时我们认为这个服务不可靠，不应该在重试期间内再次被调用，
           因此这个时候不会返回该 stickyInvoker。
           如果 selected 不包含 stickyInvoker，此时还需要进行可用性检测，
           比如检测服务提供者网络连通性等。当可用性检测通过，才可返回 stickyInvoker，
    ...
    private Invoker<T> reselect(LoadBalance loadbalance, Invocation invocation,
        ...
        // 把selected 列表中可用的挑出来放在reselectInvokers列表中
        if (selected != null) {
        ...
        if (!reselectInvokers.isEmpty()) {
            // 对reselectInvokers列表进行负载均衡选择，并返回选择结果
            return loadbalance.select(reselectInvokers, getUrl(), invocation);
        }
    ...
    public Result invoke(final Invocation invocation) throws RpcException {
        ...
        //用于列举 Invoker 其实就是列举出所有已经由路由规则过滤过的服务提供者
